...
def test_te_total_dim():
    ...
    df = make_2d_data()
    tps = TensorProductSmooth(
        {"x1": df["x1"].values, "x2": df["x2"].values}, ["x1", "x2"], k_values=[5, 6]
    )
...
def test_te_basis_finite():
    ...
    df = make_2d_data()
    tps = TensorProductSmooth(
        {"x1": df["x1"].values, "x2": df["x2"].values}, ["x1", "x2"], k_values=[8, 8]
    )
...
def test_te_penalty_symmetric():
    ...
    df = make_2d_data(60)
    tps = TensorProductSmooth(
        {"x1": df["x1"].values, "x2": df["x2"].values}, ["x1", "x2"], k_values=[5, 5]
    )
