...
from pymgcv.distributions import (
    ...
    TweedieFamily, NegativeBinomialFamily, InverseGaussianFamily
)
...
@pytest.mark.parametrize("family, y, offset, weights", [
    (GaussianFamily(),
     np.random.randn(_n), None, None),
    (PoissonFamily(),
     np.random.poisson(2.0, _n).astype(float), None, None),
    (BinomialFamily(),
     np.random.binomial(1, 0.5, _n).astype(float), None, None),
    (GammaFamily(),
     np.random.gamma(2.0, 2.0, _n), None, None),
    (TweedieFamily(),
     np.random.gamma(2.0, 2.0, _n), None, None),
    (NegativeBinomialFamily(),
     np.random.negative_binomial(2, 0.5, _n).astype(float), None, None),
