...
class MAGICOptimizer:
    ...
    def _optimize_reml(
        ...
        for outer_it in range(max_outer_iter):
            ...
            reml_score, grad_log_lambda, hess_log_lambda = reml_obj.objective_gradient_hessian(
                beta, self.lambda_log
            )
            ...
            if not (
                np.isfinite(reml_score)
                and np.all(np.isfinite(grad_log_lambda))
                and np.all(np.isfinite(hess_log_lambda))
            ):
                if verbose:
            ...
            if len(d_log_lambda) == 0:
                ...
                np.max(np.abs(alpha * d_log_lambda)) < outer_tol
                and np.linalg.norm(grad_log_lambda) < 1e-3
            ):
                self.converged = True
                if verbose:
