...
def _get_fitted_info(model) -> dict:
    ...
    if X is not None and y is not None and beta is not None and family is not None:
        ...
        offset_vec = (offset.offset_vector()
        ...
        if i > 0:
            ...
            if delta_df <= 0:
                ...
                if test == 'F':
                    ...
                    p_val = float(1 - f_dist.cdf(f_stat, delta_df, df2)) if np.isfinite(f_stat) else np.nan
                    row['stat'] = f_stat
                    row['p-value'] = p_val
                else:  # Chisq
                    chi_stat = delta_dev / max(phi, 1e-10)
                    p_val = float(1 - chi2.cdf(chi_stat, delta_df)) if np.isfinite(chi_stat) else np.nan
                    row['stat'] = chi_stat
                    row['p-value'] = p_val
