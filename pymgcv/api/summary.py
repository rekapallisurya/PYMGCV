...
def summary(model: GAM, detailed: bool = True) -> str:
    ...
    for nm, coef, se in zip(param_names, beta_param, se_param):
        ...
        stars = (
            "***"
            if p_val < 0.001
            else "**" if p_val < 0.01 else "*" if p_val < 0.05 else "." if p_val < 0.1 else ""
        )
        lines.append(f"{nm:30s} {coef:12.6f} {se:10.6f} {t_val:10.4f} {p_val:10.6f} {stars}")
    lines.append("-" * 70)
    lines.append("")
...
class ModelSummary:
    ...
    def _compute_stats(self) -> None:
        ...
        self.model_stats = {
            ...
            "null_deviance": (
                self.model.null_deviance if hasattr(self.model, "null_deviance") else None
            ),
