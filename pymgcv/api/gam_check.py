...
def _get_residuals(model, type: str = "deviance") -> np.ndarray:
    ...
    if type == "response":
        ...
        raise ValueError(
            f"Unknown residual type: {type!r}. Use 'deviance', 'pearson', or 'response'."
        )


def k_check(model, subsample: int | None = None) -> pd.DataFrame:
    ...
    offset = (
        mm_obj.offset_vector()
        if (mm_obj and hasattr(mm_obj, "offset_vector"))
        else np.zeros(len(y))
    )
    ...
    for j, (basis_obj, sl) in enumerate(zip(smooth_bases, smooth_indices)):
        ...
        edf_j = (
            edf_per_smooth.get(label, {}).get("edf", k - 1)
            if isinstance(edf_per_smooth.get(label), dict)
            else float(edf_per_smooth.get(label, k - 1))
        )
