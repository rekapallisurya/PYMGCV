...
class ThinPlateSpline:
    ...
    def __init__(
        ...
        if self.k > self.n:
            self.k = self.n
            warnings.warn(
                f"Basis dimension k={self.k} >= n={self.n}. Reducing k to {self.n}.", UserWarning
            )
        ...
        self.poly_coef: np.ndarray | None = None
    ...
    def _select_knots_quantile(self, k: int) -> np.ndarray:
        ...
        if self.d == 1:
            ...
            try:
                ...
                kmeans = KMeans(n_clusters=k, random_state=42, n_init=10, max_iter=100, tol=1e-6)
                kmeans.fit(self.X)
    ...
    def _construct_basis(self) -> None:
        ...
        aug = np.vstack(
            [np.hstack([S_rr, P_k]), np.hstack([P_k.T, np.zeros((p_dim, p_dim))])]
        )  # shape (k+d+1, k+d+1)
    ...
    def predict_basis(self, X_new: np.ndarray) -> np.ndarray:
        ...
        if X_new.shape[1] != self.d:
            raise ValueError(f"X_new has dimension {X_new.shape[1]}, expected {self.d}")
        ...
        p_dim = self.d + 1
        P_new = np.column_stack([np.ones(len(X_new)), X_new])  # shape (n_new, d+1)
