...
def format_pymgcv_output(
    ...
    if hasattr(model, 'coefficients') and hasattr(model, 'se'):
        ...
        for i, (c, s) in enumerate(zip(coef, se)):
            ...
            output.append(
                f"{name:20} {c:12.6f} {s:12.6f} {t_val:10.4f} "
                f"{p_val:12.4e} {sig}"
            )
    ...
    if hasattr(model, 'edf') and hasattr(model, 'smooth_terms'):
        ...
        for smooth_name, edf_val in model.edf.items():
            ...
            output.append(
                f"{smooth_name:20} {edf_val:8.2f} {10:8.2f} "
                f"{chi2:10.2f} {p_val:12.4e}"
            )
    ...
    if hasattr(model, 'aic'):
        output.append(f"AIC:              {model.aic:.6f}" if np.isfinite(model.aic) else "AIC:              N/A")
    
    if hasattr(model, 'gcv'):
        output.append(f"GCV score:        {model.gcv:.6f}" if np.isfinite(model.gcv) else "GCV score:        N/A")
