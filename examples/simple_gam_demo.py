...
def print_gam_summary(
    ...
    print(
        "Link function: logit"
        if family == "binomial"
        else "Link function: identity" if family == "gaussian" else f"Link function: {family}"
    )
...
def main() -> None:
    ...
    try:
        ...
        print_gam_summary(
            ...
            aic=0.0,
            gcv=0.12,
            coefficients={"Intercept": (0.0215, 0.0518, 0.416, 0.678)},
            edf_summary={"s(x)": (2.45, 2.99)},
        )
