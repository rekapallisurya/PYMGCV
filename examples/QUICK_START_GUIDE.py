...
def example_4_mixed_model():
    ...
    try:
        ...
        y = 0.5 + 2 * z + np.sin(6 * np.pi * x) + np.random.normal(0, 0.15, n)
...
def example_5_advanced_settings():
    ...
    try:
        ...
        y = np.sin(x) + 0.1 * x + np.random.normal(0, 0.25, n)

        data = pd.DataFrame({"x": x, "y": y})

        # Create model with custom settings
        model = GAM(
            formula="y ~ s(x, k=15)", family="gaussian", verbose=True  # Show convergence info
        )

        print("\nFitting with verbose output...")
        model.fit(data, max_iter=100, tol=1e-7)  # Maximum PIRLS iterations  # Convergence tolerance
...
def example_6_predictions_with_ci():
    ...
    try:
        ...
        n = 100
        x = np.linspace(0, 1, n)
        y = 3 + 2 * x + np.sin(4 * np.pi * x) + np.random.normal(0, 0.2, n)
...
def example_7_model_inspection():
    ...
    try:
        ...
        n = 150
        x = np.linspace(0, 2, n)
        y = np.exp(0.1 * x) + np.sin(3 * x) + np.random.normal(0, 0.3, n)
